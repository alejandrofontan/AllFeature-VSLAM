...
- **[`System`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L21)** (constructor) — `Optimizer::LoadParameters`, `Tracking::LoadParameters`, `LocalMapping::LoadParameters`, `LoopClosing::LoadParameters`, `PlaceCellSettings::Load`, `FeatureMatcher::setDescriptorDistanceThresholds`, then the constructors of `Map`, `FrameDrawer`, `MapDrawer`, `Tracking`, `LocalMapping`, `LoopClosing`, `Viewer`
- **[`Track`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L340)** — `Tracking::grab_image`; honours a pending [`reset`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L389)
- **[`GBA`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L678)** → **[`Shutdown`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L395)** → **[`SaveKeyFrameTrajectoryVSLAMLAB`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L482)**, **[`SavePointCloudVSLAMLAB`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L561)** (via the free helper [`write_ply_binary`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L534)), **[`SavePlaceCellDiagnostics`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L429)** — the end-of-run sequence the entry points call in this order
- Queries from the Viewer: [`GetPlaceCell`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L456), [`SetSequenceInfo`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L461), [`GetModalityDescription`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L467), plus the inline `GetPlaceCellSettings`, `GetSequenceImageCount`, `GetFramesProcessedCount` in `include/System.h`
- Not called by anything in this repo today: [`TrackStereo`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L233), [`TrackRGBD`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L287) (both `std::terminate` stubs), [`MapChanged`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L376), [`GetTrackingState`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L587), [`GetTrackedMapPoints`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L593), [`GetTrackedKeyPointsUn`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L599), [`SaveStatistics`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L605), [`setImageSize`](https://github.com/alejandrofontan/AllFeature-VSLAM/blob/main/src/System.cc#L673)
...
- rebinds Pangolin to the main viewer window (the viewer thread may have left the placecell window bound) and, with a placecell store and `PlaceCell.PrintProfile: 1`, flushes stdout and prints placecell's profile table on stderr — complete only now that every placecell caller has stopped.
...
```
- no-op without a placecell store (`vpr: none`) or with `PlaceCell.Dump: 0`. Otherwise `PlaceCell::dump(directory)` (kernel `.npy` raw + centred, `views.csv`, the recorder CSVs, `profile.csv`) followed by a windowless `placecell::viz::Visualizer` render of the three plots saved as PNGs at full size (`windows = false`, `max_hz = 0`, `kernel.centred` and `history.last_n` from the `PlaceCell.Visualize*` settings). Any exception is one `AF_WARN` and the function returns; nothing else is affected.
...
```
- non-owning pointer to the placecell store, null without `vpr: megaloc`; the Viewer builds its placecell panels on it. `System` outlives the Viewer ([`Shutdown`](#shutdown) joins its thread), so the raw pointer is safe. The inline `GetPlaceCellSettings` returns the `PlaceCell.*` settings the panels obey.
...
| `PlaceCell.*` | see `include/PlaceCellSettings.h` | [`System`](#system) via `PlaceCellSettings::Load`; `PrintProfile` in [`Shutdown`](#shutdown), `Dump`/`VisualizeCentred`/`VisualizeHistoryLastN` in [`SavePlaceCellDiagnostics`](#saveplacecelldiagnostics) | placecell logger level, profiler, recorder, dump and visualizer switches |
