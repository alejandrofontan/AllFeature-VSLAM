...
namespace AF_VSLAM {
    ...
    public:
        ...
        virtual MatcherType getMatcherType()     const = 0;
        virtual const std::string& getSettingsYamlFile()   const = 0;
        virtual const Eigen::Matrix<float,3,1>& getColor() const = 0;
        virtual float descriptor_distance(const cv::Mat &, const cv::Mat &) const { return 0.0f; };
        virtual std::shared_ptr<FeatureExtractor> createExtractor(
            std::shared_ptr<FeatureExtractorSettings>) const { return nullptr; };
    };
}
