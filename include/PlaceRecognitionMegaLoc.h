 ...
 *
 * With `LocalMapping.InformationKernel: megaloc` the same store also answers the keyframe
 * information queries (KeyframeInformationMegaLoc, KeyframeInformation.h): that query
 * embeds every tracked frame and caches the embedding in Frame::global_descriptor;
 * KeyFrame inherits it, so compute(KeyFrame&) stores that descriptor instead of
 * embedding the image again.
 *
 * Threading: compute() is called from the LocalMapping thread (keyframes) and the
 * tracking thread (relocalization query); placecell serialises internally. The database has its own mutex; queries take a snapshot and score
 * outside the lock.
 */
